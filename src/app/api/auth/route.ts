...
export async function POST(req: NextRequest) {
  ...
  // Support DEV_ADMIN_PASSWORD for local development convenience
  if (process.env.NODE_ENV !== 'production') {
    const dev = process.env.DEV_ADMIN_PASSWORD;
    if (dev && password === dev) return NextResponse.json({ token: password });
  }
